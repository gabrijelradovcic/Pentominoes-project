...
public class experimentsbot {
    ...
    private double w7 = -0.512;
    public boolean cascade = false;
    public experimentsbot(experimentsloop game) {
        this.game=game;
    ...
    public void positions(int pentomino, int rotation, int[][] field, int x, int y) {
        ...
        if (canPut(pieceToPlace, pentomino, x, y + 1, field) && canPut(pieceToPlace, pentomino, x, y, field) && alreadybeenthere(x,y+1)) {
    ...
    public void bestPosition() {
        ...
        for (int k = 0; k < a.size(); k++) {
            ...
            cost =w1*t+w2*height(boardcopy) + w3*bumpiness(boardcopy)+ w4*countholes2(boardcopy) + w5* counthols(boardcopy) + w6 * numberOfRowsRemoved +w7*numberOfRowsRemoved;
    ...
    public static int numberOfPits(int[][] field) {
        int pitCounter = 0;
        int sumColumn = 0;
