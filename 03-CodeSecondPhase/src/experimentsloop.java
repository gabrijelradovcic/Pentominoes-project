...
public class experimentsloop {

    public static double averagefitness = 0;
    ...
    public void setBot(experimentsbot bot) {
        this.bot = bot;
    }
    ...
    public void game() {
            ...
            public void run() {
                ...
                if (botplays) {
                    if (placeinarray == currentplaceinarray) {
                        bot.step();
                        while (experimentsbot.methodover == false) {
                ...
                if (canPut(pieceToPlace, x, m, n, oldfield)) {
                    ...
                    if (m == -1) {
                        experimentmain.scores[order] = fullscore;
                        fitness += fullscore;
                        order++;
                        System.out.println(fullscore+" "+order);
                        if (order ==999) {
                            experimentmain.writeCSV(experimentmain.scores);

                            averagefitness = fitness / 1000;
        ...
        newmove.schedule(move, 50, (long) Math.floor(2 / 2f));
    }
