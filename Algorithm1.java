...
{	
    ...
    {   y=fixing(piece, pentID, x, y, field);
        for(int i = 0; i < piece.length; i++) // loop over x position of pentomino
        {
            for (int j = 0; j < piece[i].length; j++) // loop over y position of pentomino
            {
    ...
    public static boolean outOfTheBounds2(int [][]pieceToPlace,int pentID,int x,int y, int[][] field){
        int t=y;
        y=fixing(pieceToPlace, pentID, x, y, field);
        if(field.length<(x+pieceToPlace.length)){
            return false;
        }
        if(field[0].length<(y+pieceToPlace[0].length)){
    ...
    public static boolean branching(int [][] field){
        if(isFull(field)){
            ui.setState(field);
        ...
        int x;
        int y;
            x = findEmptySquare(field)[0];
            y = findEmptySquare(field)[1];
        ...
        for(int i=0;i<input.length;i++){
            ...
            for(int j = 0; j<PentominoDatabase.data[pentID].length; j++){
                ...
                if(canPut(pieceToPlace,pentID, x, y, field)) {
                    int[][] backupfield = copyOf(field);
                    addPiece(field,pentID, pieceToPlace, pentID, x, y);
                    ui.setState(field);
                    if(branching(field)){ 
				        return true;
                    }
    ...
    public static boolean branching2(int [][] field){
        if(isFull(field)){
            ui.setState(field);
            System.out.println("solution found");
            return true;
        ...
        int x;
        int y;
            x= findEmptySquare(field)[1];
            y = findEmptySquare(field)[0];
        ...
        for(int i=0;i<input.length;i++){
            ...
            for(int j = 0; j<PentominoDatabase.data[pentID].length; j++){
                ...
                if(canPut2(pieceToPlace,pentID, x, y, field)) {
                    int[][] backupfield = copyOf(field);
                    addPiece2(field,pentID, pieceToPlace, pentID, x, y);
                    ui.setState(field);
                    if(branching2(field)){ 
				        return true;
                    }
